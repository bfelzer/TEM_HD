...
if((ag.fert1950flag == 1) && (ag.state == 1 || ag.state == 3) && pdm == 120) {
#ifdef CALIBRATE_TEM
   ag.fertn = 15.0;
#endif
}
if((ag.fert1950flag == 0) && (ag.state == 1 || ag.state == 3) && pdm == 120) {
#ifdef CALIBRATE_TEM
   ag.fertn = 2.0;
#endif
}
...
//    }
  ...
  soil.setNINPUT( ag.getNSRETENT() + ag.getVOLAN() );
  soil.setNLOST( 0.0);
...
//if(pdm == 120) {cout << "ninput = " << soil.getNINPUT() << " " << ag.fertn << " " << atms.getNDEP() << endl;}
...
//  soil.setNINPUT( soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP());
  }
  else
  {
...
//  veg.setVEGNINP(veg.getVEGNINP()/2.0);
  ...
  denitr = soil.getDENITR(veg.cmnt) * (0.01*microbe.getGMIN() + veg.getDENITR());
  soil.setNLOST( soil.getNLOST() +  denitr );
...
//                        (atms.getRAIN() + soil.getSNOWINF() - soil.getEET() ),
     ...
     soil.setNLOST( soil.getNLOST() + soil.getLCHDIN());

//if(soil.getNLOST() < 0) { cout << "soil.getNLOST = " << denitr << " " << soil.getLCHDIN() << endl;}

//cout << "denitr = " << 0.01*microbe.getGMIN() +  veg.getDENITR() << " " << soil.getLCHDON() << " " << pstate[I_DOC] << " " << soil.getLCHDIN() << endl;
//    soil.setLCHDIN(soil.getNLOST());

/*   if(initFlag == 0)
   {
     nin = soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed;
     nout = soil.getNLOST() + soil.getLCHDON() + nprod;
//cout << "nin/nout = " << nin << " " << nout << " " << nseed << " " << nprod << endl;
   if(nout < nin)
   {
...
/*   if(initFlag == 0)
   {
   if(soil.getNLOST() + soil.getLCHDON() < soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed)
   {
      soil.setNLOST(soil.getNLOST() + (soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed) - (soil.getNLOST() + soil.getLCHDON()));
   }
   if(soil.getNLOST() + soil.getLCHDON() > soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed)
   {
      soil.setNINPUT(soil.getNINPUT() + ((soil.getNLOST() + soil.getLCHDON()) - ((soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed))));
   } 
   }
   else
{ 
...
//   (used during calibration process)
    soil.setNLOST( (soil.getNINPUT()
                   + microbe.getNETNMIN()
                   - veg.getNUPTAKE()) );
    if(soil.getNLOST() < 0.0) { soil.setNLOST(0.0);}
  }
...
//  cout << "LEAFC = " << pstate[I_LEAFC] << " " << pdstate[I_LEAFC] << " " << veg.getALLOCLC() << " " << veg.getRMLEAF() << " " << veg.getLTRLC() << " " << pstate[I_LABILEC] <<  endl;
  ...
  pdstate[I_LABILEC] = veg.getGPP()
                  ...
                  - veg.getALLOCSEEDC()
                  - veg.getRGRWTH();
//if(ag.state == 1 && (ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt))) { cout << "LabileC = " << pdm << " " << pdyr << " " << pstate[I_LABILEC] << " " << pdstate[I_LABILEC] << " " << veg.getGPP() << " " << veg.getALLOCLC() << " " <<  veg.getALLOCSC() << " " << veg.getALLOCRC() << " " << veg.getALLOCSEEDC() << " " << veg.getRGRWTH() << endl;
//cin.get(); }
...
//cout << "nirr = " << atms.getNIRRN() << endl;
//cout << "outside conditional " << soil.getAVLH2O() << " " << soil.getAWCAPMM() << " " << veg.getPESOILW() << " " << atms.getPREC() << endl;
 if((soil.getAVLH2O() < .5 * soil.getAWCAPMM()) && (veg.getPESOILW() > 0.5 * atms.getPREC()))
{
  veg.setESOILMMMO(0.5 * atms.getPREC());
...
/*if(veg.getDENITR() > 0.0)
{
cout << "denitr  = " << 0.01*microbe.getGMIN() << " " << veg.getDENITR() << " " << y[I_LABILEN] << " " << pdm << endl;
cin.get();
} */  
...
// set grassland/tundra values
   if(veg.cmnt == 2 || veg.cmnt == 3 || veg.cmnt == 7 || veg.cmnt == 8 || veg.cmnt >= 14)
  {
   ag.setVCONVERT(1.0);
   ag.setPROD10PAR(0.0);
   ag.setPROD100PAR(0.0);
...
//
//   BSF NJ Pinelands disturbance
//
  if(initFlag == 1 && pdyr == 2 && ichrt == 1 && (pdm >= 182 && pdm <= 202))
  {
    ag.setSLASHC(0.5*y[I_LEAFC]);
    ag.setSLASHN(0.5*y[I_LEAFN]);
    ag.setVOLAC(0.5*y[I_LEAFC]);
    ag.setVOLAN(0.5*y[I_LEAFN]);
    prevy[I_LEAFC] = y[I_LEAFC] = 0.0;
    prevy[I_LEAFN] = y[I_LEAFN] = 0.0;
    cout << "volac = " << ag.getVOLAC();
...
//if(ag.state == 1 && fallow == 0) {
//cout << "fallow = " << fallow << " " << pdyr << " " << pdm << endl;
//cin.get(); }
	...
	//                  * atms.getNDAYS( pdm )) );
	    ag.setGROWDD( ag.getGROWDD()
			  + ((atms.getTAIR() - ag.getGDDMIN(ag.cmnt))));
//    cout << "growdd = " << atms.getTAIR() << " " << ag.getGDDMIN(ag.cmnt) << " " <<  ag.getGROWDD() << " " << pdyr << " " << pdm << endl;
//    cin.get();
	  }
	  else
	  {
		...
		&& atms.getTAIRN() <= ag.getTKILL(ag.cmnt)
                && fallow == 0 )
	    {
   cout << "in frost" << pdyr << " " << pdm << " " << ag.getGROWDD() << " " << ag.getGDDSEED(ag.cmnt) << endl;
   cin.get();
	      #ifdef DEBUG_CTEM
		move(DEBUG_ROW,1);
...
//	  if(atms.getPREC() < 200)
	  if(atms.getPREC() < 200/30.5)
	   {
//	    ag.irrigate = 200.0-atms.getPREC();
	    ag.irrigate = 200.0/30.5-atms.getPREC();
...
//cout << "time = " << pdyr << " " << pdm << " " << initFlag << " " << atms.getCO2() << endl;
...
if(pdm == 335) {
//cout << "vegc = " <<  pdyr << " " << y[I_LEAFC] << " " <<  y[I_SAPWOODC] << " " << y[I_HEARTWOODC] << " " << y[I_ROOTC] << " " <<  y[I_SEEDC] << " " << y[I_LABILEC] << " " << veg.getVEGC() << endl;
//cout << "vegc = " <<  pdyr << " " << veg.getVEGC() << " " << atms.getPAR() << endl;
}
...
//    ag.harvest( pdm, y[I_SEEDC], y[I_SEEDN], veg.getVEGC() - ag.getCROPSEEDC( ag.cmnt ), veg.getVEGN() - ag.getCROPSEEDSTON( ag.cmnt ) );
    ...
    fallow = 1;
    harpdm = pdm;
//   cout << "in harvest" << pdyr << " " << pdm << " " << fallow << " " << y[I_SEEDC] << " " << veg.getVEGC() << " " <<  y[I_SEEDC]/veg.getVEGC() << " " << veg.getVEGC() - y[I_SEEDC] << " " << ag.getSTUBBLEC() <<  endl;
//   cin.get();
...
//    cout << "stubble = " << ag.getSTUBBLEC() << " " << ag.getSTUBBLEN() << endl;
//    cin.get();
    ag.setGROWDD( ZERO );
    harcnt = harcnt + 1; 
...
if(ag.state == 1 && ((ag.getGROWDD() > (ag.getGDDSEED(ag.cmnt) - 5)) && (ag.getGROWDD() < (ag.getGDDSEED(ag.cmnt) + 5))) && pdm < harpdm)
{
      y[I_LABILEC] = ag.getCROPSEEDC( ag.cmnt );
      y[I_LABILEN] = ag.getCROPSEEDSTON( ag.cmnt );
      fallow = 0;
//      cout << "planting" << " " << pdyr << " " << pdm << " " << ag.getGROWDD() <<  " " << ag.getGDDSEED(ag.cmnt) << endl;
//      cin.get();
}  
...
//ag.setNoGrazing();
//
//  BSF Cut lawn once per week (every 7 days) in the growing season
//
 if( 3 == ag.state && (ag.getGROWDD() >= ag.getGDDSEED(ag.cmnt) && ag.getGROWDD() <= ag.getGDDHARVST(ag.cmnt)) && (pdm%7) == 0.0) // turflawn
  {
...
//
  ...
  nce = ag.getCFLUX() - ag.getTOTPRODDECAYC() - ag.getVOLAC() + cseed - ag.getCROPPRODC();
...
//  soil.setNLOST( (y[I_NLST]
  ...
  soil.setNLOST( (soil.getNLOST()
                  ...
                  + ag.getCROPRESIDUEFLXN()
                  + nprod));
  soil.setNINPUT( soil.getNINPUT() + nseed );
//  soil.setNINPUT( ( ag.fertn) );
#endif
//if(ag.state == 2) {cout << "nlost = " << soil.getLCHDON() << " " << soil.getLCHDIN() << " " << denitr << " " << pdm << " " << pdyr << " " << soil.getNLOST() << endl;}
//cout << "nin = " << y[I_NFIXN] << " " << y[I_NFIXS] << " " << atms.getNDEP()/365000. << " " << soil.getNINPUT() << endl;
//cin.get();
...
ctol = (ctol/100.0)*(veg.getVEGC()+y[I_SOLC]);
ntol = (ntol/100.0)*(veg.getVEGN()+y[I_SOLN]);
...
if(ag.state == 0)
{
//cout << "C equil values = " <<  ctol << " " << fabs((veg.getVEGC()+y[I_SOLC])-(vegceq+soilceq)) << " " << fabs(veg.getVEGC()-vegceq) << " " << fabs(y[I_SOLC]-soilceq) << endl;  
//cout << "n equil values = " << ntol << " " << fabs( soil.yrnin - soil.yrnlost ) << " " << fabs((veg.getVEGN()+y[I_SOLN]))-(vegneq+soilneq) << " " << fabs((veg.getVEGN()-vegneq)) << " " << fabs((y[I_SOLN]-soilneq)) << endl;
...
//  soil.yrnin = soil.yrnin*(1.0 - favg) + 12.0*(y[I_NINP] + y[I_NFIXN] + y[I_NFIXS])*favg;
  ...
  yrnfix = yrnfix*(1.0-favg) + 365.0*(y[I_NFIXN]+y[I_NFIXS])*favg;
  atms.yrndep = atms.yrndep*(1.0-favg) + 365.0*(atms.getNDEP()/365000.)*favg;
...
//  soil.yrnlost = soil.yrnlost*(1.0 - favg) + 12.0*(y[I_NLST])*favg;
  ...
  soil.yrlchdin = soil.yrlchdin*(1.0 - favg) + 365.0*(soil.getLCHDIN())*favg;
  soil.yrlchdon = soil.yrlchdon*(1.0 - favg) + 365.0*(soil.getLCHDON())*favg;
  yrdenitr = yrdenitr*(1.0 - favg) + 365.0*denitr*favg;

/*if(pdm >= 364 && ag.state == 2) {
cout << "yrnin = " << soil.yrnin << " " << yrnfix << " " << atms.yrndep << " " << pdm << endl;
cout << "yrnlost = " << soil.yrnlost << " " << soil.yrlchdin << " " << soil.yrlchdon << " " << yrdenitr << " " << soil.getNLOST() << endl;
if(soil.getLCHDIN() < 0.0) { cout << "lchdin = " << soil.getLCHDIN() << endl;}
cin.get(); 
} */    
