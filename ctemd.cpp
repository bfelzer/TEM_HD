...
enum tcalkey { T_TOL,      T_TAVG,     T_CO2,      T_AVLNFLAG, 
               ...
               T_CNSOL,
               T_CNLTR,    T_MNFIX,    T_DENITR,    T_NLOSS,
...
int main()
{
  ...
  tem.tauavg = 1.0;
//  BSF  Add initFlag = 0 so calibration similar to equilibration
  tem.initFlag = 0.0;
...
//    cout << "first CYCLE = " << CYCLE << " " << xdyr << endl;
...
//
//   BSF FIX to calculate correct CYCLE
//
    if( spinflag == 2 ) { cdyear = (de_startyear - clm.startyr) + (xdyr%de_nyears); }
    else { cdyear = 0; }

    ndays[0] = 31;
    if(((clm.startyr + cdyear)%4) == 0.0)
    {
    ndays[1] = 29;
    }
    else
...
//    cout << "second CYCLE = " << CYCLE << " " << xdyr << " " << cdyear << endl;
...
//      if(tem.atms.getDAYL() == 0)
//      {
//       cout << "dayl = " << tem.atms.getDAYL() << " " << dm << " " << cdyear << " " << CYCLE << endl;
//        getchar();
//      }
...
#else
//      o3dat[dyr] = tairdat[dyr];
      o3dat[dyr].col = tairdat[dyr].col;
      o3dat[dyr].row = tairdat[dyr].row;
      o3dat[dyr].carea = tairdat[dyr].carea;
      o3dat[dyr].year = tairdat[dyr].year;
      o3dat[dyr].month = tairdat[dyr].month;
      o3dat[dyr].day = tairdat[dyr].day;
      o3dat[dyr].contnent = tairdat[dyr].contnent;
      o3dat[dyr].varname = "OZONE";
      o3dat[dyr].mon[day] = 0.0;
#endif
...
#else
      ndepdat[dyr].col = tairdat[dyr].col;
      ndepdat[dyr].row = tairdat[dyr].row;
      ndepdat[dyr].year = tairdat[dyr].year;
      ndepdat[dyr].adepval = 0.00001;
#endif
