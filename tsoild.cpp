...
//    extra = 10.0/30.0*(avlh2o - awcapmm);
//    BSF bug fix to daily model
    extra = 0.05 * (avlh2o - awcapmm);
...
//    cout << "rperc = " << rperc << " " << awcapmm << "  " << avlh2o << " " << rain << endl;
...
//  if(rrun < 0) { cout << " rrun = " << rrun << " " << rgrndh2o << endl;}

  if(rrun < 0.0) { rrun = 0.0;}
  srun = srunoff( elev,
                  tair,
                  prevtair,
                  prev2tair,
                  sgrndh2o );
  if(srun < 0.0 ) { srun = 0.0;}
...
{
   lchdin = (availn/(soilh2o+rrun+srun))*(rrun+srun);
/*  if( soilh2o > ZERO )
  {
...
lchdin *= nloss[pdcmnt];
/*if(lchdin < 0.0) {
cout << "lchdin in soil= " << lchdin << " " <<  availn << " " << soilh2o << " " << rrun << " " << srun << " " << nloss[pdcmnt] << endl;
cin.get();
} */
};
...
//cout << "soiltext = " << pctclay << " " << pctsand << " " << pdcmnt << endl;

//cout << "awcapmm = " << awcapmm << " " << fldcap << " " << wiltpt << " " << rootz << " " << pota << " " << potb << endl;
